...
auth = QWidget()
auth.setWindowTitle('Авторизація')
auth.resize(300, 100)
auth_layout = QVBoxLayout()
auth_btn = QPushButton("OK")
auth_line = QLineEdit("")
auth_line.setPlaceholderText("Введіть пароль")
auth_layout.addWidget(auth_line)
auth_layout.addWidget(auth_btn)
auth.setLayout(auth_layout)
auth_line.setEchoMode(QLineEdit.Password)
i = 0
def check():
    global i
    if auth_line.text() == "ponosmamonta":
        notes_win.show()
        auth.hide()
        sound = pyglet.media.load('pvz.mp3', streaming=False)
        sound.play()
    else:
        i += 1
        if i == 3:
            auth.hide()
        show_mess("Вийди отсюда, розбійник!")

auth.show()
auth_btn.clicked.connect(check)
...
def show_mess(text):
    ...
    msg.setText(text)
    msg.setWindowTitle('Розумні замітки')
    msg.exec_()
...
def save_note():
    if list_notes.selectedItems():
        ...
        file = open("nazva.json", "w")
        json.dump(notes, file, sort_keys=True, ensure_ascii=False)
        file.close()
        print(notes)
    else:
        show_mess("Замітка для збереження не вибрана!")
...
def del_note():
    if list_notes.selectedItems():
        ...
        with open("nazva.json", "w") as file:
            json.dump(notes, file, sort_keys=True, ensure_ascii=False)
        print(notes)
    else:
        show_mess("Замітка для вилучення не обрана!")
...
def add_tag():
    if list_notes.selectedItems():
        ...
        show_mess("Замітка для додавання тега не обрана!")
...
def del_tag():
    if list_tags.selectedItems():
        ...
        show_mess("Тег для вилучення не обраний!")
